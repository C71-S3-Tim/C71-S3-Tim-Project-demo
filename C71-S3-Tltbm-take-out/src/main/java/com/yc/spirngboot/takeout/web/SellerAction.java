...
public class SellerAction {
	...
	public String shop_intro(Model m,Integer seller_id,HttpSession hs)  {
		...
		System.out.println("======"+seller.getDistrict());
	...
	public String member_collect(Model m, Integer id,HttpSession hs) {
		
		User user=(User) hs.getAttribute("loginedUser");
		Collect co=new Collect();
		co.setsId(id);
		co.setuId(user.getId());
		CollectExample  cex=new CollectExample();
		cex.createCriteria().andUIdEqualTo(user.getId()).andSIdEqualTo(id);
		List<Collect> collects=ce.selectByExample(cex);
